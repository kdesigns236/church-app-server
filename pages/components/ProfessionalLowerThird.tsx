...
const ProfessionalLowerThird: React.FC<ProfessionalLowerThirdProps> = ({ config }) => {
  ...
  const mainBarStyle = {
    background: `linear-gradient(135deg, ${config.mainBarColor} 0%, ${darken(
     ...
     color: darken(config.mainBarColor, 90) // Ensure text color contrasts with background
  };
   const mainTextStyle = {
    ...
    <>
      <style>{`
        .lower-third-wrapper {
            ...
            max-width: 1200px;
            perspective: 1000px;
            transform: scale(0.6); /* Scaled down for video preview */
        ...
        /* Logo container with 3D diamond */
        .logo-wrapper {
            position: absolute;
            top: 50%;
            left: 40px;
