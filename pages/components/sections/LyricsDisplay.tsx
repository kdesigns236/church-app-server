...
const LyricsDisplay: React.FC<LyricsDisplayProps> = ({ config, setConfig }) => {
  ...
  const navigateVerse = (direction: 'next' | 'prev') => {
    ...
    setConfig(prev => {
      const newIndex =
        direction === 'next' ? prev.verseIndex + 1 : prev.verseIndex - 1;
      ...
      {config.song && (
        <ControlRow label="Verse Preview">
          <div className="p-2 bg-gray-900 border border-gray-700 rounded-md h-24 overflow-y-auto scroll-container">
            {config.song.verses.map((_, index) => (
              <p
                key={index}
                className={`p-1 rounded cursor-pointer ${
                  config.verseIndex === index
                    ? 'bg-blue-900/50'
                    : 'hover:bg-gray-700'
                }`}
                onClick={() =>
                  setConfig(prev => ({ ...prev, verseIndex: index }))
                }
              >
                Verse {index + 1}
              </p>
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <ControlRow label="Text Align">
          <select
            ...
            onChange={handleValueChange}
            className="w-full p-2 bg-gray-900 border border-gray-700 rounded-md h-10"
            disabled={config.animationStyle === 'scroll'}
          >
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex space-x-2">
          <button
            ...
            disabled={!config.song || config.verseIndex === 0}
          ...
          <button
            ...
            disabled={
              !config.song ||
              config.verseIndex >= (config.song?.verses.length || 0) - 1
            }
            className="px-3 py-2 bg-gray-700 rounded-md hover:bg-gray-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed text-lg font-bold"
          >
            →
