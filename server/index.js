...
const swSource = `
const STATIC='static-v6',DATA='data-v6',MEDIA='media-v6';
...
if(/(?:^|\.)firebasestorage\.app$/i.test(host)||/(?:^|\.)appspot\.com$/i.test(host)||host==='firebasestorage.googleapis.com'||host==='storage.googleapis.com'){
  e.respondWith((async()=>{const cache=await caches.open(MEDIA);const cached=await cache.match(e.request);const fetchP=fetch(e.request).then(r=>{try{cache.put(e.request,r.clone())}catch{}return r}).catch(()=>null);if(cached){fetchP&&fetchP.catch(()=>{});return cached;}if(fetchP){const r=await fetchP;if(r)return r;}return new Response('',{status:504})})());
  return
}
