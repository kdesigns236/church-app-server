    ...
    if (/^https?:\/\//i.test(b)) {
      try {
        ...
        if (m && m[1]) {
          ...
          if (parts[0]) b = String(parts[0]);
        } else if (/\.storage\.googleapis\.com$/i.test(u.hostname)) {
          b = u.hostname.replace(/\.storage\.googleapis\.com$/i, '');
        }
      } catch {}
    }
    if (b.includes('/')) {
...
async function ensureFirebaseAltMediaTokenUrl(file, bucketName, objectPath) {
  const base = `https://firebasestorage.googleapis.com/v0/b/${bucketName}/o/${encodeURIComponent(objectPath)}`;
  ...
  let token = String(rawTokens || '').split(',')[0].trim();

  if (!token) {
    token = generateFirebaseStorageDownloadToken();
    const nextMeta = Object.assign({}, (meta && meta.metadata) ? meta.metadata : {});
    nextMeta.firebaseStorageDownloadTokens = token;
    await file.setMetadata({ metadata: nextMeta });
  }
...
const swSource = `
const STATIC='static-v7',DATA='data-v7',MEDIA='media-v7';
...
self.addEventListener('activate',e=>{e.waitUntil(caches.keys().then(keys=>Promise.all(keys.map(k=>{if(k!==STATIC&&k!==DATA&&k!==MEDIA)return caches.delete(k)}))).then(()=>self.clients.claim()))});
self.addEventListener('fetch',e=>{const u=new URL(e.request.url);if(e.request.method!=='GET')return;if(e.request.mode==='navigate'){e.respondWith(fetch(e.request).then(r=>{const c=r.clone();caches.open(STATIC).then(cc=>cc.put('/index.html',c));return r}).catch(()=>caches.match('/index.html')));return}
const host=u.hostname;
if(/(?:^|\.)firebasestorage\.app$/i.test(host)||/(?:^|\.)appspot\.com$/i.test(host)||host==='firebasestorage.googleapis.com'||host==='storage.googleapis.com'){
  const isRange=e.request.headers&&e.request.headers.get('range');
  if(isRange){e.respondWith(fetch(e.request));return}
  e.respondWith((async()=>{const cache=await caches.open(MEDIA);const cached=await cache.match(e.request);const fetchP=fetch(e.request).then(r=>{try{if(r&&r.ok&&r.status===200){cache.put(e.request,r.clone())}}catch{}return r}).catch(()=>null);if(cached){fetchP&&fetchP.catch(()=>{});return cached;}if(fetchP){const r=await fetchP;if(r)return r;}return new Response('',{status:504})})());
  return
}
if(u.pathname==='/api/sync/data'){e.respondWith((async()=>{const cache=await caches.open(DATA);const cached=await cache.match(e.request);const fetchP=fetch(e.request).then(r=>{cache.put(e.request,r.clone());return r}).catch(()=>null);if(cached){fetchP&&fetchP.catch(()=>{});return cached;}const [sRes,scRes]=await Promise.all([cache.match('/api/sermons'),cache.match('/api/site-content')]);if(sRes||scRes){let s=[],sc={};try{s=sRes?await sRes.json():[]}catch{}try{sc=scRes?await scRes.json():{}}catch{}const body=JSON.stringify({sermons:s,siteContent:sc});return new Response(body,{headers:{'Content-Type':'application/json'}});}if(fetchP){const r=await fetchP;if(r)return r;}return new Response('{}',{status:503});})());return}
if(u.pathname.startsWith('/api/')){e.respondWith(caches.open(DATA).then(cache=>cache.match(e.request).then(cached=>{const fetchP=fetch(e.request).then(r=>{cache.put(e.request,r.clone());return r});return cached?(fetchP.catch(()=>{}),cached):fetchP})));return}if(u.pathname.startsWith('/bible/')){e.respondWith(caches.match(e.request).then(cached=>cached||fetch(e.request).then(r=>{const rc=r.clone();caches.open(STATIC).then(cc=>cc.put(e.request,rc));return r})));return}if(u.pathname.includes('/assets/')||/\.(js|css|png|jpg|jpeg|gif|svg|webp|woff2?)$/i.test(u.pathname)){e.respondWith(caches.match(e.request).then(cached=>cached||fetch(e.request).then(r=>{const rc=r.clone();caches.open(STATIC).then(cc=>cc.put(e.request,rc));return r})));return}});
`;
